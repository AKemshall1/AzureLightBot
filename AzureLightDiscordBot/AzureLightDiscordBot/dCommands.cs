...
namespace AzureLightDiscordBot
{
    class dCommands:BaseCommandModule
    {
        ...
        public async Task EquipmentGuide(CommandContext ctx, string guideType)
        {
            ...
            Console.WriteLine(@"images\" + filename);
            if (File.Exists(@"images\" + filename))
            {
                await ctx.Channel.SendFileAsync(@"images\" + filename).ConfigureAwait(false);
            }
            else
                await ctx.Channel.SendMessageAsync("That equipment guide doesn't exist. Accecpted format: !equip aa/araux/bbaux/bbgunmain/bbgunaux/cagun/clcaaux/clgun/cvaux/ddaux/ddgunmain/planes/sub/torp").ConfigureAwait(false);
        }
        //---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        public async Task shipGeneral(CommandContext ctx, string boat)
        {
            ...
            if (File.Exists(@"wikia\Ships\" + shipCalled))
            {
                ...
                await ctx.Channel.SendMessageAsync("That ship doesn't exist. Multiple word ships must have underscores (e.g. ning_hai). Submarines contain no dashes (e.g i58)").ConfigureAwait(false);
        ...
        public async Task ReturnStageLevel(CommandContext ctx, string stage)
        {
            ...
            if (jo.SelectToken(stage) != null)
            {
                ...
                await ctx.Channel.SendMessageAsync("That stage does not exist. Accepted format : !stage 2-4, !stage 2-4hard. SOS stages are listed under x-5.").ConfigureAwait(false); ;
